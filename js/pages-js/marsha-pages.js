...
const show = (showPhotos) => {
  const item = overlayPhotosMarsha[showPhotos];
...
for (let i = 0; i < allPhotos.length; i++) {
  allPhotos[i].addEventListener("click", () => {
    ...
    const item = overlayPhotosMarsha[i];
    imgOverlay.src = item.photos;

    nextBtn.addEventListener("click", () => {
      photoIndex++;
      if (photoIndex > overlayPhotosMarsha.length - 1) {
        photoIndex = 0;
      }
      show(photoIndex);
    });
    ...
    prevBtn.addEventListener("click", () => {
      photoIndex--;
      if (photoIndex < 0) {
        photoIndex = overlayPhotosMarsha.length - 1;
      }
      show(photoIndex);
    });
